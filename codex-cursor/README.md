...
The terminal stays on an inline dashboard while the proxy runs. It shows:

- the Cursor base URL and API key hint
- request totals, active requests, and the latest request status
- cumulative input/output token usage reported by the upstream response
- a small recent event log

Keyboard controls:

- `r` resets the displayed request and token totals
- `q` exits cleanly
